...
public class ColorPickerDialog extends Dialog implements
    ...
    public ColorPickerDialog(Context context, int theme, int initialColor,
            int androidColor, int cyanideColor) {
        super(context, theme);
        mInitialColor = initialColor;
        mAndroidColor = androidColor;
        mCyanideColor = cyanideColor;
        if (mAndroidColor != 0x00000000 && mCyanideColor != 0x00000000) {
    ...
    public boolean onMenuItemClick(MenuItem item) {
        if (item.getItemId() == R.id.palette_cyanide) {
            mPalette = PALETTE_CYANIDE;
            mColorTransitionAnimator.start();
            return true;
            ...
            return true;
        } else if (item.getItemId() == R.id.reset_cyanide) {
            mColorPicker.setColor(mCyanideColor, true);
            return true;
        }
        return false;
    }
